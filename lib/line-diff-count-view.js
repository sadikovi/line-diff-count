...
export default class LineDiffCountView {
  ...
  constructor() {
    ...
    this.repoSubscriptions = new WeakMap();
  ...
  registerEditor(editor) {
    ...
    editor.onDidDestroy(() => {
      ...
      this.unsubscribeFromRepository(editor);
    });
  ...
  subscribeToRepository(editor) {
    ...
    if (repo) {
      const repoSubscription = new CompositeDisposable(
      ...
      this.repoSubscriptions.set(editor, repoSubscription);
      this.subscriptions.add(repoSubscription);
    }
  }

  unsubscribeFromRepository(editor) {
    if (!this.repoSubscriptions) return;
    const repoSubscription = this.repoSubscriptions.get(editor);
    if (repoSubscription) {
      repoSubscription.dispose();
      this.subscriptions.remove(repoSubscription);
      this.repoSubscriptions.delete(editor);
    }
  }
  ...
  visible() {
    return !!this.lineDiff && this.lineDiff.style.display != 'none';
  }
